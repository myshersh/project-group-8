...
- `change-contact <name> <old_phone> <new_phone>`: Updates an existing phone number for a contact.
- `show-phone <name>`: Shows all phone numbers for a specific contact.
...
- `create-demodata`: Install exemple of dadbase of AddressBook for using different commands.
...
- `birthdays <days>`: Displays a table of all contacts who have birthdays occurring within the next inputed days between 3 and 30, including the exact recommended congratulation date.

### Notes Management

- `add-note <text>`: Creates a new text note.
- `change-note <id> <new_text>`: Overwrites the text of an existing note by its numeric ID.
- `delete-note <id1, id2, idn>`: Deletes a note (or set of notes) by its numeric IDs.
